...
import { LOAD_POST,GET_ALL,GET_ALL_FAIL, CREATE_SUCCESS,CREATE_FAIL, UPDATE,GET_POST,GET_POST_FAIL, DELETE,COMMENT_SUCCESS,COMMENT_FAIL,LIKE } from '../ActionsTypes/postConstant';
...
//updatePOst
  ...
  //commentPOst
    export const commentPost = (newText,id) => async (dispatch) => {
      try {
        const opts = {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        };
            const {data} = await axios.put(
              `http://localhost:5000/post/commentPost/${id}`,newText,opts);
